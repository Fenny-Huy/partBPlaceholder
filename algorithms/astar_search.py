...
def astar(start, goal, centroids, edges, get_volume_at_edge):
    """
    ...
    while frontier:
        ...
        for (A, B, dist_km) in [e for e in edges if e[0] == current]:
            volume = get_volume_at_edge(A, B)
            speed_kmh = flow_to_speed(volume)
            travel_time = dist_km / speed_kmh * 60  # minutes
            print(f"speed_kmh: {speed_kmh:.2f} km/h, travel_time: {travel_time:.2f} min, dist_km: {dist_km:.2f} km, volume: {volume:.2f} veh/h")
